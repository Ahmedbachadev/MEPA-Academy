  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        save(
          async () => await supabase.from("hero_section").update({
            title: form.title, subtitle: form.subtitle,
            button_one_label: form.button_one_label, button_one_href: form.button_one_href,
            button_two_label: form.button_two_label, button_two_href: form.button_two_href,
            stat_students: form.stat_students, stat_courses: form.stat_courses,
            stat_seminars: form.stat_seminars, stat_success_rate: form.stat_success_rate,
          }).eq("id", form.id),
          ["site", "hero"]
        );
      }}
      className="space-y-5"
    >
      <AdminHeading title="Hero Section" description="Headline, subtitle, CTAs and stats shown on the homepage hero." />
